...
for _, query_id in ipairs(query_ids) do
    table.insert(updated_pairs, current_time)
...
if #query_ids < needed then
    ...
    for i = 1, needed - #query_ids do
        table.insert(updated_pairs, current_time)
        table.insert(updated_pairs, max_query_id + i - 1)
        table.insert(execution_result, max_query_id + i - 1)
    end
end
