...
export class BotService {
    constructor(
        ...
        @Inject('PROCESSING_SERVICE') private readonly processingClient: ClientKafka
    ) { }
    ...
    async processPayment(userId: string, invoice: string, chargeId: string) {
        ...
        // await this.user.createTransaction({
        //     userId,
        //     data: {
        //         address: data[0],
        //         source: Number(data[1]),
        //         target: Number(data[2]),
        //         route: data[3] as Symbol
        //     },
        //     lpFee: Number(data[4]),
        //     bchFees: Number(data[5]),
        //     chargeId
        // })
        ...
        this.processingClient.emit(`${data[3]}-requests`, request)
    }
}
